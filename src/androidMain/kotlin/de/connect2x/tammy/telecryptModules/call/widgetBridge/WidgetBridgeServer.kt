...
/**
 * Android-копия [WidgetBridgeServer] из desktopMain. Отличие — загрузка
 * `widget-host.html` из `AndroidManager.assets` вместо classpath-ресурса
 * (на Android resources на classpath работают иначе и `widget-host.html`
 * из desktopMain/resources в APK не попадает).
 *
 * URLs:
...
class WidgetBridgeServer(
    ...
    private fun loadHostHtmlTemplate(): String {
        return context.assets.open("widget-host.html").use { stream ->
